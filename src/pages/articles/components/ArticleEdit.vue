        ...
        <el-form-item label="封面图片" prop="cover_image">
          <div class="cover-upload">
            <el-upload
              ...
              accept="image/*"
            >
              <img v-if="formData.cover_image" :src="formData.cover_image" class="cover" />
...
// 表单数据
const formData = reactive<CreateArticleRequest & { location: { latitude: number, longitude: number, address: string } }>({
  title: '',
  content: '',
  summary: '',
  category: '',
...
const fillFormData = (article: Article) => {
  ...
  enableLocation.value = !!(article.location?.latitude && article.location?.longitude)
}
...
const handleSave = async () => {
  ...
  try {
    ...
    saving.value = true
       ...
       location: enableLocation.value && formData.location.latitude && formData.location.longitude
         ? formData.location
         : undefined
     }

    if (isEdit.value && props.articleId) {
      const res = await updateArticleApi(props.articleId, requestData as UpdateArticleRequest)
      if (res.success) {
